...
const MedicalandLedger = async (first_name, last_name, res, recordErrorMsg) => {
  try {
    if (recordErrorMsg) {
      ...
      fs.readFile(ledgerFilePath, (err, data2) => {
        ...
        if (!res.headersSent) {
          res.status(200).json({
            Status: "True",
            message: "Patient Records sent successfully",
            MedicalData:recoredError,
